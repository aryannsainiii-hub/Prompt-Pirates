...
export const RAW_CANDIDATE_DATA = [
  {
    member: {
      id: 'CAND-001',
      name: 'Sarah Johnson',
...
export const CANDIDATE_PROFILES: CandidateProfile[] = RAW_CANDIDATE_DATA.map((raw) => {
  const completedDays = raw.missions
    .filter((m) => m.passed)
    .map((m) => m.day);

  const passedTitles = raw.missions
    .filter((m) => m.passed)
    .map((m) => m.title);

  const expLabel = raw.member.yearsExperience === 0 
    ? `Entry Level / Intern • ${raw.member.education}`
    : `${raw.member.yearsExperience} yrs exp • ${raw.member.education}`;

  return {
    id: raw.member.id,
    name: raw.member.name,
    role: raw.member.jobRole,
    background: `${raw.member.jobRole} (${raw.member.yearsExperience} yrs exp, ${raw.member.education}). Completed ${raw.signals.missionsCompleted} cohort missions across ${raw.signals.commitDays} commit days (${raw.signals.missionsFirstTry} on first try).`,
    experienceLevel: expLabel,
    completedDays: completedDays.length > 0 ? completedDays : [1, 3, 7, 12],
    targetAreas: passedTitles.slice(0, 4),
    notes: `Cohort Status: ${raw.member.status}. Commit Days: ${raw.signals.commitDays}, Completed Missions: ${raw.signals.missionsCompleted}, First-Try Passes: ${raw.signals.missionsFirstTry}.`,
    member: raw.member,
...
export function getCandidateById(candidateId: string): CandidateProfile | undefined {
  return CANDIDATE_PROFILES.find((c) => c.id === candidateId || c.id.toLowerCase() === candidateId.toLowerCase());
}
